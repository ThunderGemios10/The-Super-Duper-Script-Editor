...
class EditorForm(QtGui.QMainWindow):
  ...
  def loadDirectory(self, directory, clear_similarity = True, selected_file = None):
    ...
    if directory[:7] == "anagram":
      ...
      try:
        self.script_pack.load_dir(directory, common.editor_config.umdimage_dir)
    ...
    for script in reversed(self.script_pack):
      ...
      # "umdimage" is how it's stored in the database, regardless of where the
      # user's umdimage directory is.
      self.similarity_db.queue_query_at_top(os.path.join("umdimage", self.script_pack.get_real_dir(), basename))
  ...
  def loadAnagram(self, anagram):
    ...
    path = os.path.join(common.editor_config.umdimage_dir, anagram)
  ...
  def insertLine(self):
    ...
    dir = os.path.join("umdimage", self.script_pack.get_real_dir())
    filename = os.path.basename(self.script_pack[self.cur_script].filename)
  ...
  def updateUI(self):
    ...
    if scene_info.speaker in common.CHAR_IDS:
      self.ui.lblSpeaker.setText(common.CHAR_IDS[scene_info.speaker])
    else:
      self.ui.lblSpeaker.setText("N/A")
    ...
    sprite_char = scene_info.sprite.char_id
    if sprite_char in common.CHAR_IDS:
      sprite_text = common.CHAR_IDS[sprite_char]
      self.ui.lblSprite.setText(sprite_text)
    
    else:
      self.ui.lblSprite.setText("N/A")
    ...
    ### MAP
    ###################################################
    map_name = get_map_name(scene_info.room)
    
    self.ui.lblRoom.setToolTip("Room ID: %d" % scene_info.room)
    if not map_name == None:
      self.ui.lblRoom.setText(map_name)
    else:
    ...
    if not voice == None:
      if scene_info.voice.char_id in common.CHAR_IDS:
        voice_char = common.CHAR_IDS[scene_info.voice.char_id]
      else:
        voice_char = ""
      
      self.ui.lblVoice.setText(voice_char)
      self.ui.btnVoice.setEnabled(True)
      
      voice_tooltip += "\nFile: %d" % voice
    else:
      self.ui.lblVoice.setText("N/A")
      self.ui.btnVoice.setEnabled(False)
      
    if scene_info.bgm != -1:
      voice_tooltip += "\n\nBGM ID: %d" % scene_info.bgm
    else:
      voice_tooltip += "\n\nBGM ID: N/A"
    ...
    ###################################################
    if scene_info.special == common.SCENE_SPECIAL.option:
      self.ui.lblSpecial.setText("Options: Option %d" % scene_info.extra_val)
      ...
      self.ui.lblSpecial.setText("Reaction Talk")
  ...
  def showDupes(self):
    ...
    filename  = os.path.basename(self.script_pack[self.cur_script].filename)
    filename  = os.path.join("umdimage", directory, filename)
    filename  = os.path.normpath(os.path.normcase(filename))
    ...
    if not dupes == None:
      ...
      for file in dupes:
        if not file == filename:
          parsed_file = tree.consolidate_path(file)
  ...
  def updateSimilarView(self, tree_item):
    ...
    filename = os.path.join(common.editor_config.umdimage_dir, expanded_dir, file)
    ...
    self.ui.txtSimilarTrans.setPlainText(script_file.translated)
    self.ui.txtSimilarOrig.setPlainText(script_file.original)
  ...
  def showOpenMenu(self):
    ...
    if self.open_menu.current_dir != None and not self.directory == self.open_menu.current_dir and self.askUnsavedChanges():
  ...
  def saveChanges(self):
    ...
    progress = QProgressDialog("Saving...", QtCore.QString(), 0, len(self.script_pack), self)
    ...
    # The base name (without "umdimage") of all the files being saved.
    files   = []
    ...
    for index, script in enumerate(self.script_pack):
      ...
      filename = os.path.basename(script.filename)
      filename = os.path.join("umdimage", dir, filename)
      filename = os.path.normpath(os.path.normcase(filename))
      ...
      if dupes == None:
        ...
        files[-1].append(filename[9:])
        file_count += 1
        continue
      
      # This includes the original file itself.
      for dupe in dupes:
        files[-1].append(dupe[9:])
        file_count += 1
    ...
    for file_set in files:
      for file in file_set:
        source = os.path.join(common.editor_config.umdimage_dir, file)
    ...
    for index, script in enumerate(self.script_pack):
      ...
      for file in file_set:
        ...
        target      = os.path.join(common.editor_config.umdimage_dir, file)
  ...
  def playVoice(self):
    ...
    voice_id = get_voice_file(self.script_pack[self.cur_script].scene_info.voice)
    
    if voice_id == None:
      return
    
    voice_file = os.path.join(common.editor_config.voice_dir, "%04d.at3" % voice_id)
    
    if not os.path.isfile(voice_file):
      QtGui.QMessageBox.warning(self, "Could Not Find File", u"Could not locate\n%s" % voice_file)
      return
    
    self.qsound_voice = QSound(voice_file)
    self.qsound_voice.play()
  ...
  ##############################################################################
  def stopVoice(self):
    if not self.qsound_voice == None:
      self.qsound_voice.stop()
  ...
  def updateStatusDirInfo(self):
    self.ui.statusLabelDirInfo.setText("File: %d / %d" % (self.cur_script + 1, len(self.script_pack.script_files)))
  ...
  def changedTranslated(self):
    ...
    if not translated == self.script_pack[self.cur_script].translated:
      ...
      self.script_pack[self.cur_script].translated = translated
      
      self.changed[self.cur_script] = True
      
      for dupe in self.internal_dupes[self.cur_script]:
        self.script_pack[dupe].translated = translated
  ...
  def changedScriptFile(self, index):
    ...
    self.ui.txtTranslated.setPlainText(self.script_pack[index].translated)
    
    cursor_match = self.CURSOR_RE.search(self.script_pack[index].translated)
  ...
  def addDupe(self, node):
    ...
    active_file = os.path.join("umdimage", active_dir, active_file)
    active_file = os.path.normpath(os.path.normcase(active_file))
  ...
  ##############################################################################
  ### @fn   buildArchives()
  ### @desc Create umdimage.dat and umdimage2.dat from our data folders and
  ###       output an EBOOT to use those archives.
  ##############################################################################
  ...
  ##############################################################################
  ### @fn   importUmdimage()
  ### @desc Imports a directory into umdimage. <3
  ##############################################################################
  def importUmdimage(self):
  ...
  ##############################################################################
  ### @fn   importUmdimage2()
  ### @desc Imports a directory into umdimage2. <3
  ##############################################################################
  def importUmdimage2(self):
  ...
  ##############################################################################
  ### @fn   importOther()
  ### @desc Imports a directory into another directory. <3
  ##############################################################################
  def importOther(self):
    ...
    target_dir = QtGui.QFileDialog.getExistingDirectory(self, caption = "Select a target directory", directory = common.editor_config.last_import_target)
    if not target_dir == "":
  ...
  def showCurrentInExplorer(self):
    directory = self.script_pack.get_real_dir()
    directory = os.path.join(common.editor_config.umdimage_dir, directory)
  ...
  def showNodeInExplorer(self, node):
    ...
    if not node == None:
      ...
      directory = os.path.join(common.editor_config.umdimage_dir, directory, filename)
      dir_tools.show_in_explorer(directory)
...
<li>Silk Icon Set: Copyright Mark James; Licensed under the Creative Commons Attribution 2.5 License; <a href="http://www.famfamfam.com/lab/icons/silk/">Website</a></li>
  ...
  def checkForErrors(self):
    ...
    diffs.set_folders(base_dir, common.editor_config.umdimage_dir, errors)
    diffs.exec_()
