...
RE_UMDIMAGE = re.compile(ur"umdimage[\\\/]?", re.UNICODE | re.S)
RE_LIN      = re.compile(ur"((script_pak_)?e.*?)\.lin.*?\1\.pak", re.UNICODE | re.S)
def consolidate_path(path):
  
  consolidated = RE_UMDIMAGE.sub("", path)
  consolidated = RE_LIN.sub("\\1.lin", consolidated)
